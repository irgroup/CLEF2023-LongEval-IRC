...
Example:
    Train a system on the train slice of the train topics and create a run on the test slice of the train topics ::
        ...
        $ python -m systems.LambdaMART-LGB_LETOR_USE --index WT  --train
    
    Train a system on the train slice of the train topics and create a run on the test topics (submission run) ::

        $ python -m systems.LambdaMART-LGB_LETOR_USE --index WT
"""
import json
import os
...
def main(args):
    ...
    if args.train:
        ...
        run_tag = tag("BM25+LambdaMART_LGB_LETOR_USE-train", "WT")
    else:
        # use the test topics
        _, topics, _ = setup_system(args.index, train=False)
        run_tag = tag("BM25+LambdaMART_LGB_LETOR_USE", "WT")
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Run BM25+LambdaMART_LGB_LETOR_USE")
    parser.add_argument(
        "--index",
        type=str,
        default="WT",
